# Build Blueprint — how this repo is built

This repository is built **phase by phase, under an automated execute-and-verify loop**, from the
plan in [`phases/`](./phases) and the knowledge base in [`docs/`](./docs). This file is the entry
point for any AI agent (or human) running that build. For what the SDK *is* and how to use it, see
[`README.md`](./README.md).

> Progress so far: P1 Core, P2 Designer, P3 Save Path and P5 Visual Parity are **done** (all
> machine gates green); P4 Live & Pilot is partially blocked on backend/infra work.
> [`STATE.md`](./STATE.md) is the authoritative ledger.
